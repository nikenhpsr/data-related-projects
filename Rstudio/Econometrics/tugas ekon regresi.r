...
e1=y-(x%*%b)
s2=(t(e1)%*%e1)/(n-k)
#simpangan baku kekeliruannya sekitar 36.28474
...
R=cor(x0)
VIF=diag(solve(R))
#semua x VIF kurang dari 5 maka , dari semua tidak perlu ada yang perlu dikhawatirkan ttg multikolinearitas
...
DFFcri<- 2*sqrt(p/n)
abs(DFF)>1
#berarti kota semarang sangat berpengaruh terhadap fitted model (yi) PDRB kota semarang

## Cook's Distance
D <- (e^2/p/s2[1,1])*(h/diag(I-H)^2)
Dcri <- qf(0.5,p,n-p)
D>=Dcri
#berarti tidak ada kota/kabupaten yang mendominasi terhadap fitted model (y) PDRB
